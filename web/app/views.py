...
def lab11_microblog():
    if request.method == 'POST':
        ...
        validated = True
        validated_dict = dict()
        valid_keys = ['name', 'avatar_url','message', 'email']
        ...
        if validated:
            app.logger.debug('validated dict: ' + str(validated_dict))
            # if there is no id: create a new blogentry entry
            if not id_:
                entry = BlogEntry(**validated_dict)
                app.logger.debug(str(entry))
                db.session.add(entry)
            # if there is an id already: update the blogentry entry
            else:
...
def lab12_login():
    if request.method == 'POST':
        ...
        if not next_page or url_parse(next_page).netloc != '':
            next_page = url_for('lab10_phonebook')
